    ...
    public SseEmitter streamTest(HttpServletResponse response) {
        ...
        executor.execute(() -> {
            try {
                ...
                // 发送连接成功事件
                emitter.send(SseEmitter.event().name("connected").data("连接已建立!"));

                Thread.sleep(1000);

                // 步骤1：查看现有的BI报表
                // 先发送标题
                emitter.send(SseEmitter.event().name("step-title").data(createStepTitle(1, "查看现有的BI报表，匹配符合条件")));

                // 延迟100ms后发送内容
                Thread.sleep(1000);
                emitter.send(SseEmitter.event().name("content").data(createStep1Content()));

                Thread.sleep(1500);

                // 步骤2：尝试查找数据集
                // 先发送标题
                emitter.send(SseEmitter.event().name("step-title").data(createStepTitle(2, "尝试通过SQL数据集查询，找到合适的数据集")));

                // 延迟100ms后发送内容
                Thread.sleep(1000);
                emitter.send(SseEmitter.event().name("content").data(createStep2Content()));

                Thread.sleep(1500);

                // 步骤3：查看报表元数据
                // 先发送标题
                emitter.send(SseEmitter.event().name("step-title").data(createStepTitle(3, "查看数据集元数据")));

                // 延迟100ms后发送内容
                Thread.sleep(1000);
                emitter.send(SseEmitter.event().name("content").data(createStep3Content()));

                Thread.sleep(1500);

                // 步骤4：生成SQL
                // 先发送标题
                emitter.send(SseEmitter.event().name("step-title").data(createStepTitle(4, "根据元数据和需求生成SQL")));

                // 延迟100ms后发送内容
                Thread.sleep(1000);
                emitter.send(SseEmitter.event().name("content").data(createStep4Content()));

                Thread.sleep(1500);

                // 步骤5：查询数据结果
                // 先发送标题
                emitter.send(SseEmitter.event().name("step-title").data(createStepTitle(5, "根据SQL查询数据")));

                // 延迟100ms后发送内容
                Thread.sleep(1000);
                emitter.send(SseEmitter.event().name("content").data(createStep5Content()));

                Thread.sleep(1500);

                // 步骤6：生成图表
                // 先发送标题
                emitter.send(SseEmitter.event().name("step-title").data(createStepTitle(6, "今日生产线进度与质量对比图")));

                // 延迟100ms后发送内容
                Thread.sleep(1000);
                emitter.send(SseEmitter.event().name("content").data(createStep6Content()));

                Thread.sleep(1000);

                // 发送完成事件
                emitter.send(SseEmitter.event().name("completed").data("数据传输完成!"));
    ...
    public SseEmitter streamChatAI(HttpServletResponse response) {
        ...
        executor.execute(() -> {
            try {
                ...
                // 要输出的文本
                String answer = "今天的生产进度情况如下：\n\n" + "A线 - 高速组装：已完成目标的88%，合格率99.12%\n"
                        + "B线 - 自动化包装：已完成目标的80%，合格率99.54%\n"
                        + "C线 - 精密加工：已完成目标的85%，合格率98.76%\n"
                        + "D线 - 测试验证：已完成目标的75%，合格率99.87%\n"
                        + "E线 - 定制生产：已完成目标的65%，合格率99.92%\n\n"
                        + "总体来看，生产线都在按计划推进，质量控制良好。A线进度最快，E线目前进度较慢，可能需要关注。";

                // 字符流输出
                char[] chars = answer.toCharArray();

                for (char c : chars) {
                    emitter.send(SseEmitter.event().name("char").data(String.valueOf(c)));
                ...
                // 发送完成事件
                Thread.sleep(100);
                emitter.send(SseEmitter.event().name("completed").data("文本生成完成!"));
